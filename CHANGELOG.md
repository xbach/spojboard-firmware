...
- **A reset can now keep your WiFi credentials, your panel setup, or both.** Two tick-boxes beside the reset field: keep WiFi and the device rejoins your network on the same address instead of starting a hotspot, so you carry on configuring it from the same browser tab; keep the panel arrangement and wiring and the display comes back looking the same instead of needing to be described again. Both start unticked, so a plain reset is still a full factory reset
